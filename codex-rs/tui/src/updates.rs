    ...
    let latest_version = match update_action::get_update_action() {
        ...
        _ => {
            ...
            let latest_tag_name = releases
                .into_iter()
                .find_map(|r| extract_version_from_latest_tag(&r.tag_name).ok().map(|v| (r.tag_name, v)))
                .map(|(_, v)| v)
                .ok_or_else(|| anyhow::anyhow!("No valid release tags found"))?;
            latest_tag_name
        }
    };
...
fn extract_version_from_latest_tag(latest_tag_name: &str) -> anyhow::Result<String> {
    ...
    Err(anyhow::anyhow!("Failed to parse latest tag name '{latest_tag_name}'"))
}
